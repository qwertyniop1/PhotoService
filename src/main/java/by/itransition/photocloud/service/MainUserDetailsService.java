...
public class MainUserDetailsService implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String email) throws UsernameNotFoundException {
        by.itransition.photocloud.persistance.model.User user = userDao.findByEmail(email);
        if (user == null) throw new UsernameNotFoundException("No user found with username: "+ email);
        List<GrantedAuthority> authorities = buildUserAuthority(user.getUserRoles());
        return buildUserForAuthentication(user, authorities);
    }

    private User buildUserForAuthentication(by.itransition.photocloud.persistance.model.User user,
